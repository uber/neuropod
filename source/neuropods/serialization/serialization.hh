...
using deserialize_fn_t = std::function<std::shared_ptr<NeuropodValue>(boost::archive::binary_iarchive &, NeuropodTensorAllocator &)>;
...
template <typename T>
bool register_serializable(
    std::function<void(const T &, boost::archive::binary_oarchive &)> serialize_fn,
...
std::shared_ptr<NeuropodValue> deserialize(std::istream &in, Params &&... params)
{
    ...
    if (version != SERIALIZATION_VERSION)
    {
        NEUROPOD_ERROR("This serialized tensor was created with a different version of Neuropod serialization code."
            "Expected version " << SERIALIZATION_VERSION << " but got " << version);
    }
