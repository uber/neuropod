...
public:
        ...
        : uuid_(uuid_generator())
    {
        ...
        shm_ = stdx::make_unique<ipc::shared_memory_object>(ipc::create_only, get_key_from_uuid(uuid_).c_str(), ipc::read_write);
    ...
    SHMBlock(const shm_block_id * block_id)
    {
        ...
        shm_ = stdx::make_unique<ipc::shared_memory_object>(ipc::open_only, get_key_from_uuid(uuid_).c_str(), ipc::read_write);
        ...
        if (block_->process_refcount == 0)
        {
            ...
            NEUROPOD_ERROR("Tried getting a pointer to an existing chunk of memory that has a refcount of zero: " << uuid_);
        }
        ...
        if (expected_reuse_count != block_->reuse_count)
        {
            ...
            NEUROPOD_ERROR(
                "Tried loading a block of memory that went out of scope in the creating process. "
                "Ensure that blocks of SHM stay in scope until they are loaded by the receiving process. "
                "UUID: " << uuid_ << ". Reuse count: " << block_->reuse_count << ". Expected reuse count: " << expected_reuse_count
            );
...
SHMAllocator::SHMAllocator()
    : unused_pool_(stdx::make_unique<UnusedPool>())
{
}
