...
void run_in_and_out_of_process(const std::string &neuropod_path, T fn)
{
    ...
    auto multiprocess_time = time_lambda<std::chrono::microseconds>(100, 1000, [&multiprocess_neuropod, &fn]() {
        fn(*multiprocess_neuropod);
    });

    neuropods::Neuropod inprocess_neuropod(neuropod_path);
    auto inprocess_time = time_lambda<std::chrono::microseconds>(100, 1000, [&inprocess_neuropod, &fn]() {
