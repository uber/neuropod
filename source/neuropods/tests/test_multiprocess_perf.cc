...
void run_in_and_out_of_process(const std::string &neuropod_path, T fn)
{
    ...
    auto multiprocess_time     = time_lambda<std::chrono::microseconds>(
        100, 1000, [&multiprocess_neuropod, &fn]() { fn(*multiprocess_neuropod); });

    neuropods::Neuropod inprocess_neuropod(neuropod_path);
    auto                inprocess_time =
        time_lambda<std::chrono::microseconds>(100, 1000, [&inprocess_neuropod, &fn]() { fn(inprocess_neuropod); });

    std::cout << "Times: in process - " << inprocess_time << " microseconds, out of process - " << multiprocess_time
...
TEST(test_multiprocess_perf, object_detection)
{
    ...
    run_in_and_out_of_process("neuropods/tests/test_data/dummy_object_detection/",
                              [&some_image_data](neuropods::Neuropod &neuropod) {
...
TEST(test_multiprocess_perf, small_inputs)
{
    ...
    run_in_and_out_of_process("neuropods/tests/test_data/dummy_small_input_model/",
                              [&some_data](neuropods::Neuropod &neuropod) {
                                  neuropods::NeuropodValueMap input_data;
