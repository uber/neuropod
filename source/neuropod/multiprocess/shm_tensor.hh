...
private:
    ...
    // where the last dimension is the size of the max length string
...
public:
    ...
    void set(const std::vector<std::string> &data)
    {
        ...
        // Space for the length of the item
        max_len_ += sizeof(int64_t);
        ...
        {
            // Set the item size
            *(reinterpret_cast<int64_t *>(pos)) = item.size();

            // Copy in the data
            std::copy(item.begin(), item.end(), pos + sizeof(int64_t));
...
protected:
    ...
    {
        auto    data_start = data_->get_raw_data_ptr() + index * max_len_;
        int64_t len        = *(reinterpret_cast<int64_t *>(data_start));
        data_start += sizeof(int64_t);
        return std::string(data_start, data_start + len);
    }
};
