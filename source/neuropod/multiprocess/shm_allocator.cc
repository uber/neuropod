...
std::map<RawSHMHandle, std::shared_ptr<void>> loaded_cache;
std::mutex                                    loaded_cache_mutex;

// Try to get a raw block from the loaded cache
void maybe_get_from_cache(const RawSHMHandle &handle, std::shared_ptr<void> &raw_block)
{
    std::lock_guard<std::mutex> lock(loaded_cache_mutex);
    auto                        item = loaded_cache.find(handle);
    if (item != loaded_cache.end())
    {
        raw_block = item->second;
        loaded_cache.erase(item);
    }
}
...
std::unordered_multimap<size_t, RawBlockWrapper> created_cache;
std::mutex                                       created_cache_mutex;

// Maybe get an unused raw block from the created cache
// Increments the refcount and reuse count of the returned block (if any)
void maybe_get_from_cache(size_t requested_size, std::shared_ptr<void> &raw_block, SHMBlockIDInternal &id)
{
    std::lock_guard<std::mutex> lock(created_cache_mutex);
    auto                        range = created_cache.equal_range(requested_size);
...
std::shared_ptr<void> SHMAllocator::allocate_shm(size_t size_bytes, SHMBlockID &block_id)
{
    ...
    maybe_get_from_cache(requested_size, raw_block, id);
    ...
    return std::shared_ptr<void>(block->data, [block, raw_block, requested_size, id](void *unused) {
        ...
        std::lock_guard<std::mutex> cache_lock(created_cache_mutex);
        RawBlockWrapper             wrapper = {raw_block, id.block_handle};
        created_cache.emplace(requested_size, wrapper);
    });
}
...
std::shared_ptr<void> SHMAllocator::load_shm(const SHMBlockID &block_id)
{
    ...
    maybe_get_from_cache(handle, raw_block);
    ...
    // Create a shared pointer to the underlying data with a custom deleter
    // that keeps the block alive. Add the block to the cache on destruction.
    return std::shared_ptr<void>(block->data, [block, raw_block, handle](void *unused) {
        // Lock the block's mutex
        ipc::scoped_lock<ipc::interprocess_mutex> lock(block->mutex);
        ...
        std::lock_guard<std::mutex> cache_lock(loaded_cache_mutex);
        loaded_cache[handle] = raw_block;
    });
}
